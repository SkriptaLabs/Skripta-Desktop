  ...
  return (
    <div class="p-4 flex flex-col gap-4">
      ...
      <div class="flex items-center justify-between">
        <h2 class="text-base font-medium">Meine Notizen</h2>
        <button
      ...
      <Show when={creating()}>
        <form onSubmit={handleCreate} class="flex gap-2 p-3 border border-border rounded bg-muted">
          ...
          />
          <button type="submit" class="text-sm px-3 text-foreground/70 hover:text-foreground">Erstellen</button>
          <button type="button" onClick={() => setCreating(false)} class="text-sm px-2 text-foreground/40 hover:text-foreground">✕</button>
        </form>
      </Show>

      {/* Liste */}
      <Show when={!loading()} fallback={<p class="text-sm text-foreground/50">Lade…</p>}>
        <Show when={error()}>
          {(err) => <p class="text-sm text-red-500">{err()}</p>}
        </Show>
        <Show
          when={notes().length > 0}
          fallback={<p class="text-sm text-foreground/40">Noch keine Notizen.</p>}
        >
            ...
            <For each={notes()}>{(note) => (
              <li class="flex items-center justify-between px-3 py-2 rounded border border-border hover:bg-muted group">
                <div class="cursor-pointer flex-1 min-w-0">
                  <p class="text-sm font-medium truncate">{note.title || "(Ohne Titel)"}</p>
